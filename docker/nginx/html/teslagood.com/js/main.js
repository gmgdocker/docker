...
!(function( $ ) {
	...
	$(function() {
	    ...
	    var name_val = $('input[name="name"]'),
	        ...
	        country_val = $('input[name="country"]'),
	    ...
	    function validate(a, b) {
	        ...
	        if (!c.search(a) == 0 && c.length > 0) {
	            b.closest('.form__block').addClass('error').removeClass('valid');
	        } else if (c.length < 1){
	            b.closest('.form__block').removeClass('error').removeClass('valid');
	        } else {
	            b.closest('.form__block').removeClass('error').addClass('valid');
	        }
	
	        if(b.hasClass('search')){
	            if(b.parent().hasClass('changed')){
	                b.closest('.form__block').removeClass('error').addClass('valid');
	            } else {
	                b.closest('.form__block').addClass('error').removeClass('valid');
	            }
	    ...
	    $('.cntr-ls li').on('click',function(){
	        $('input[name=phone]').parent().addClass('valid');
	        validate(name_check, country_val);
	    });
	    ...
	    forms.on('submit',function (event) {
	        ...
	        var that = $(this),
	            ...
	            country = that.find('input[name="country"]'),
	            email = that.find('input[name="email"]');
	
	        validateOnSubmit(name_check, name) !== undefined ? valid = false : '';
	        validateOnSubmit(phone_check, phone) !== undefined ? valid = false : '';
	        validateOnSubmit(email_check, email) !== undefined ? valid = false : '';
	        validateOnSubmit(name_check, country) !== undefined ? valid = false : '';
		...
		function textUp(letter){
			...
			var timer = setInterval(function(){
				if(j<c){
					letter.text(letter.text()+a[j]);
					j=j+1;
				} else {
					letter.removeClass('after');
					clearInterval(timer);
				} 
			},45);
		};

		function textUpStart(selector,time){
		...
		textUpStart($('.triangle__img--text p:first-child()'),4000);
		textUpStart($('.triangle__img--text p:nth-child(2)'),5200);
		textUpStart($('.triangle__img--text p:last-child()'),6200);

		$('#main').fullpage({
			lockAnchors: false,
			navigation: false,
			anchors:['disp__first', 'disp__sec','disp__third'],
			css3: true,
			scrollingSpeed: 700,
			scrollOverflow: true,
			onLeave: function(index, nextIndex, direction){
				if($('.disp__sec').hasClass('active') && !$('.disp__sec').hasClass('show')){
					$('.disp__sec').addClass('show');
				}
			}
		});
		...
		if($('[type=search]').val().length > 0){
			$('[type=tel]').parent().addClass('valid');
		}
		...
		$('.main--btn').on('click',function(){
			$('.fixed-form').addClass('show');
		});

		$('.fixed-form--close').on('click',function(){
			$('.fixed-form').removeClass('show');
		});
})( jQuery );